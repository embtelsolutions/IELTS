                ...
                <div class="category-lists job">
                   ...
                   <ul>
                      @foreach ($scats as $key => $scat)
                        <li class="single-category {{$service->scategory->id == $scat->id ? 'active' : ''}}"><a href="{{route('front.services', ['category' => $scat->id, 'term'=>request()->input('term')])}}">{{convertUtf8($scat->name)}}</a></li>
                      @endforeach
                   </ul>
                </div>
             </div>
